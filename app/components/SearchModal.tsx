...
const suggestions = [
  ...
  { emoji: '👤', label: 'Freelance',      hint: 'outils pour indép.',      query: 'freelance' },
  { emoji: '💡', label: 'Gratuit',        hint: 'sans abonnement',         query: 'gratuit' },
]

type ResultType = 'categorie' | 'guide' | 'outil' | 'article'
...
export default function SearchModal() {
  ...
  // Preloaded static data (small tables — loaded once at mount)
  const allCats  = useRef<SearchResult[]>([])
  const allGuides = useRef<Guide[]>([])

  useEffect(() => {
    Promise.all([
      supabase.from('categories').select('id,nom,slug'),
      supabase.from('cas_usage').select('id,label,slug,categorie_id'),
      supabase.from('types_entreprise').select('id,label,slug'),
    ]).then(([catsRes, casRes, typesRes]) => {
      // Categories
      allCats.current = (catsRes.data || []).map(cat => ({
        type: 'categorie' as ResultType,
        uid: `cat-${cat.id}`,
        label: cat.nom,
        catId: cat.id,
  ...
  const search = useCallback(async (q: string) => {
    ...
    // 1. Client-side filter on preloaded categories + guides
    const matchedCats: SearchResult[] = allCats.current
      .filter(c => c.label.toLowerCase().includes(ql))

    const matchedGuides: SearchResult[] = allGuides.current
      .filter(g => g.label.toLowerCase().includes(ql))
      .slice(0, 6)
      .map(g => ({ ...g, type: 'guide' as ResultType }))

    // 2. DB search for tools + articles (dynamic data)
    const [outilsRes, articlesRes] = await Promise.all([
      supabase.from('outils').select('id,nom,description,prix_mensuel,categorie_id,lien_affilie').ilike('nom', `%${q}%`).limit(3),
      supabase.from('articles').select('id,slug,titre,categorie_slug').ilike('titre', `%${q}%`).limit(3),
    ])

    const matchedOutils: SearchResult[] = (outilsRes.data || []).map(o => ({
      type: 'outil' as ResultType,
      uid: `outil-${o.id}`,
      label: o.nom,
      description: o.description,
      prix: o.prix_mensuel,
      catId: o.categorie_id,
      href: o.lien_affilie,
      isExternal: true,
    }))

    const matchedArticles: SearchResult[] = (articlesRes.data || []).map(a => ({
      type: 'article' as ResultType,
      uid: `article-${a.id}`,
      label: a.titre,
      catId: catSlugInfo[a.categorie_slug]?.id,
      href: `/blog/${a.slug}`,
    }))

    // Merge: cats → guides → articles → tools
    const merged = [
      ...matchedCats,
      ...matchedGuides,
      ...matchedArticles,
          ...
          >
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'12px', padding:'16px 20px', borderBottom: hasResults || isEmpty ? '1px solid #f1f5f9' : 'none' }}>
              ...
              <input
                ...
                placeholder="Meilleur CRM gratuit, freelance, HubSpot…"
            ...
            {hasResults && (
              <div style={{ maxHeight:'460px', overflowY:'auto' }}>
                {results.map((r, i) => {
                  ...
                  if (r.type === 'categorie') return (
                    <div key={r.uid} className={`search-result-row${isActive ? ' active' : ''}`} onMouseEnter={() => setSelectedIdx(i)} onClick={() => navigate(r)}>
                      <div style={{ width:'38px', height:'38px', background:cat?.bg || '#f8fafc', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'18px', border:`1px solid ${cat?.color || '#e2e8f0'}22` }}>
                        {cat?.emoji || '📦'}
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'7px' }}>
                          <span style={{ fontSize:'14px', fontWeight:700, color:'#0f172a' }}>{r.label}</span>
                          <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 7px', borderRadius:'999px', background:'#f1f5f9', color:'#475569', border:'1px solid #e2e8f0' }}>Catégorie</span>
                        </div>
                        <p style={{ fontSize:'12px', color:'#94a3b8', margin:'2px 0 0' }}>Tous les comparatifs</p>
                      </div>
                      <span className="search-cta" style={{ background:cat?.color || '#0f172a', color:'#fff' }}>Voir →</span>
                    </div>
                  )

                  if (r.type === 'guide') return (
                    <div key={r.uid} className={`search-result-row${isActive ? ' active' : ''}`} onMouseEnter={() => setSelectedIdx(i)} onClick={() => navigate(r)}>
                      <div style={{ width:'38px', height:'38px', background:cat?.bg || '#f5f3ff', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'16px', border:`1px solid ${cat?.color || '#7c3aed'}22` }}>
                        📄
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'7px', flexWrap:'wrap' }}>
                          <span style={{ fontSize:'14px', fontWeight:700, color:'#0f172a' }}>{r.label}</span>
                          {cat && <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 7px', borderRadius:'999px', background:cat.bg, color:cat.color, border:`1px solid ${cat.color}33` }}>{cat.emoji} {cat.name}</span>}
                        </div>
                        <p style={{ fontSize:'12px', color:'#94a3b8', margin:'2px 0 0' }}>Comparatif détaillé</p>
                      </div>
                  ...
                  if (r.type === 'article') return (
                    <div key={r.uid} className={`search-result-row${isActive ? ' active' : ''}`} onMouseEnter={() => setSelectedIdx(i)} onClick={() => navigate(r)}>
                      <div style={{ width:'38px', height:'38px', background:cat?.bg || '#f0fdf4', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'16px', border:`1px solid ${cat?.color || '#16a34a'}22` }}>
                        ✍️
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'7px', flexWrap:'wrap' }}>
                          <span style={{ fontSize:'14px', fontWeight:700, color:'#0f172a', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'320px' }}>{r.label}</span>
                          <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 7px', borderRadius:'999px', background:'#f0fdf4', color:'#16a34a', border:'1px solid #bbf7d0', flexShrink:0 }}>Article</span>
                        </div>
                        <p style={{ fontSize:'12px', color:'#94a3b8', margin:'2px 0 0' }}>Blog</p>
                      </div>
                      <span className="search-cta" style={{ background:'#16a34a', color:'#fff' }}>Lire →</span>
                    </div>
                  )
                  ...
                  return (
                    <div key={r.uid} className={`search-result-row${isActive ? ' active' : ''}`} onMouseEnter={() => setSelectedIdx(i)} onClick={() => navigate(r)}>
                      <div style={{ width:'38px', height:'38px', background:cat?.bg || '#f8fafc', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, border:`1px solid ${cat?.color || '#e2e8f0'}22` }}>
                        {logo
                          ? <img src={logo} alt={r.label} width={22} height={22} style={{ objectFit:'contain' }} onError={e => { (e.target as HTMLImageElement).style.display='none' }} />
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'7px', marginBottom:'2px', flexWrap:'wrap' }}>
                          <span style={{ fontSize:'14px', fontWeight:700, color:'#0f172a' }}>{r.label}</span>
                          {cat && <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 7px', borderRadius:'999px', background:cat.bg, color:cat.color, border:`1px solid ${cat.color}33` }}>{cat.emoji} {cat.name}</span>}
                          {free && <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 7px', borderRadius:'999px', background:'#f0fdf4', color:'#16a34a', border:'1px solid #bbf7d0' }}>Gratuit</span>}
                        </div>
                        <p style={{ fontSize:'12px', color:'#94a3b8', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', margin:0 }}>{r.description}</p>
                      </div>
            ...
            {isEmpty && (
              <div style={{ padding:'40px 20px', textAlign:'center' }}>
                ...
                <p style={{ fontSize:'13px', color:'#94a3b8' }}>Essayez : CRM, meilleur SEO gratuit, freelance, HubSpot…</p>
              </div>
            )}
